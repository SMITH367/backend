...
const discountFee = require("../services/discountFee.js")
...
router.get('/deliveryMan/wallet/info/:email', verifyToken, (req, res) => {
    ...
    jwt.verify(req.auth, 'secretKey', async (err, data) => {
        if (err) {
            ...
            try {
                const userBalance = await deliveryMan.find({
                    email: req.params.email
                }, {
                    "password": 0,
                    "_id": 0,
...
router.put('/deliveryMan/discount/fee/:id', verifyToken, (req, res) => {
    ...
    jwt.verify(req.auth, 'secretKey', async (err, data) => {
        if (err) {
            ...
            try {
                const userBalance = await deliveryMan.findOne({
                    email: req.params.id
                }, {
                    "password": 0,
                    "_id": 0,
                ...
                let newBalance = parseInt(userBalance.balance) - discountFee(parseInt(req.body.priceOfDelivery))
